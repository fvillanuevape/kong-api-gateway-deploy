...
function MultiPart:parseFile(filename)
    ...
    while true do
        ...
        if init then
            ...
            if idx == nil then
                error({
                    msg = "error finding mime boundary"
                })
            end
            ...
            if idx ~= 1 then
                ...
                if idx > 32 then
                    error({
                        msg = "too many garbage before the first mime boundary: " .. idx
                    })
                end
            ...
            if idx == nil then
                error({
                    msg = "can't find mime boundary"
                })
            elseif idx ~= 1 then
                -- parsePart() should have read to the end, therefore, when we call indexOf, it should always be 1
                error({
                    msg = "data does not start with the mime boundary, this should be a program bug"
                })
            end
        end
        ...
        if params[item.name] then
            error({
                msg = "duplicate parameter name: " .. item.name
            })
        end
...
function MultiPart:parsePart()
    ...
    if afterBoundary:len() >= 2 and afterBoundary:sub(1, 3) == "--" then
        return nil
    end
    if not afterBoundary or trim(afterBoundary):len() ~= 0 then
        error({
            msg = "no empty line following the boundary"
        })
    end
    local item = {}
    local headers = {}
    ...
    for i = 1, maxHeaders do
        if i == maxHeaders then
            error({
                msg = "too many content headers"
            })
        end
        self.reader:load(1024) -- make sure there is enough data
        ...
        if line:len() == 0 then
            ...
            if not item.filename and (not item.contentType or item.contentType == "plain/text") then
                ...
                if item.contentLength and item.contentLength ~= filesize then
                    error({
                        msg = "incorrect Content-length specified: " .. item.contentLength .. " != " .. filesize
                    })
                end
            ...
            if not headerName or not headerValue then
                error({
            ...
            if headers[headerName] then
                error({
                    msg = "duplicate header: " .. headerName
                })
            end
            ...
            if headerName == "content-disposition" then
                if headers[headerName].value ~= "form-data" then
                    error({
                        msg = "content-disposition is not form-data"
                    })
                end
                item.name = headers[headerName].attributes.name
                if not item.name then
                    error({
                        msg = "form-data without name"
                    })
                end
            ...
            elseif headerName == "content-transfer-encoding" then
                ...
                if cte ~= "base64" and cte ~= "quoted-printable" then
                    error({
                        msg = "invalid Content-Transfer-Encoding: " .. headers[headerName].value
                    })
                end
            ...
            elseif headerName == "content-length" then
                ...
                if not len then
                    error({
                        msg = "invalid Content-Length specified: " .. headers[headerName].value
                    })
                end
    ...
    error({
        msg = "the program should not reach here, must be program bug"
    })
end
