-- If you're not sure your plugin is executing, uncomment the line below andrestart Kong
-- then it will throw an error which indicates the plugin is being loaded atleast.
...
-- call local services as auth and validation strings
...
function plugin:access(config)
    ...
    local body, statusCode, headers, statusText = http.request('http://localhost:8080')
    ...
    if nil == url_cfg then
        ...
        if nil == url_cfg then
            returnError(config, "URL is not found", 404)
            return
        end
    end
    ...
    local status, err = pcall(function()
        wrapped(cfg, pathParams)
    end)

    if not status then
        returnError(config, err)
        -- returnError(config, "Invalid DTO Schema format", 404)
    end
