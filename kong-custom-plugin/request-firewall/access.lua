...
function m.isValidFile(field_attrs, name, value, nested)
    ...
    if nil == value then
        if field_attrs.allow_null == false then
            return m.fail("Null and empty filename is NOT ALLOWED: " .. utils.dump({
                name = name,
                filename = "(null)"
            }))
        else
    ...
    if field_attrs.min then
        if not value["size"] or type(value["size"]) ~= "number" or value["size"] < field_attrs.min then
            return m.fail("File size too small: " .. utils.dump({
                name = name,
                size = value["size"]
            }))
        end
    ...
    if field_attrs.max then
        if not value["size"] or type(value["size"]) ~= "number" or value["size"] > field_attrs.max then
            return m.fail("File size too large: " .. utils.dump({
                name = name,
                size = value["size"]
            }))
        end
    ...
    if field_attrs.match then
        if not value["filename"] or type(value["filename"]) ~= "string" or
            not value["filename"]:match(field_attrs.match) then
            return m.fail("Invalid filename: " .. utils.dump({
                name = name,
    ...
    if field_attrs.not_match then
        if not value["filename"] or type(value["filename"]) ~= "string" or
            value["filename"]:match(field_attrs.not_match) then
            return m.fail("Invalid filename: " .. utils.dump({
                name = name,
                filename = value["filename"]
            }))
...
function m.isValidBoolean(field_attrs, name, value, nested)
    ...
    elseif type(value) == "table" then
        ...
        for idx, v in pairs(value) do
            if type(idx) ~= "number" or not m.isValidBoolean(field_attrs, name, v, true) then
                return false
            end
        end
        return true
...
function m.isValidString(field_attrs, name, value, nested)
    ...
    elseif type(value) == "table" then
        ...
        for idx, v in pairs(value) do
            if type(idx) ~= "number" or not m.isValidString(field_attrs, name, v, true) then
                return false
            end
        end
        return true
...
function m.isValidNumber(field_attrs, name, value, nested)
    ...
    elseif type(value) == "number" then
        ...
        if ((nil == field_attrs.positive and nil == field_attrs.min) or field_attrs.positive) and value <= 0 then
        ...
        if field_attrs.enum and not utils.contains(field_attrs.enum, tostring(value)) then
    ...
    elseif type(value) == "string" then
        ...
        if field_attrs.precision and decPart and string.len(decPart) > field_attrs.precision then
        ...
        -- if both positive and min are not given, we will assume you want positive number only
        if ((nil == field_attrs.positive and nil == field_attrs.min) or field_attrs.positive) and v2 <= 0 then
            return m.fail("Number is not larger than zero: " .. utils.dump(name, value))
        end
    ...
    elseif type(value) == "table" then
        ...
        for idx, v in pairs(value) do
            if type(idx) ~= "number" or not m.isValidNumber(field_attrs, name, v, true) then
                return false
            end
        end
        return true
...
function m.validateField(config, field_attrs, name, value)
    if nil == field_attrs then
        return m.fail("Unexpected field: " .. name)
    end
